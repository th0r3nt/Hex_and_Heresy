...
# ==================================================================
# ТУМАН ВОЙНЫ: СОСТОЯНИЯ ГЕКСОВ И РАДИУСЫ ОБЗОРА
# ==================================================================


class HexVisibilityState(str, Enum):
    """
    Что фракция знает о гексе прямо сейчас.

    Состояния строго упорядочены по объему знания: неисследованный гекс
    не показывает вообще ничего, разведанный помнит ландшафт и застройку,
    а видимый отдает и то, что движется по нему в этот такт.
    """

    UNEXPLORED = "unexplored"  # черный туман: гекса как будто и нет
    FOG_OF_WAR = "fog_of_war"  # ландшафт и здания известны, армии скрыты
    VISIBLE = "visible"  # прямая видимость: гекс просматривается насквозь


# Радиусы обзора источников на глобальной карте (в гексах).
# Разведка стоит дешевле войны: обзор дают и стены, и марширующая колонна.
VISION_RADIUS_BASE: Final[int] = 2  # цитадель фракции
VISION_RADIUS_BORDER_TOWN: Final[int] = 2  # пограничный город
VISION_RADIUS_REGIONAL_HALL: Final[int] = 1  # ратуша союзной земли
VISION_RADIUS_WATCHTOWER: Final[int] = 2  # сторожевая вышка (см. gamedata)
VISION_RADIUS_ARMY: Final[int] = 1  # мобильная армия и караван рабочих
VISION_RADIUS_AMBASSADOR: Final[int] = 1  # посол в пути
