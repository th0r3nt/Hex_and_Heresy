...
async def offer(facade, world, llm, proposal_response, **overrides):
    """Готовое открытое предложение фракции игрока."""
    llm.structured_response = proposal_response(**overrides)
...
class TestSettings:
    ...
    async def test_disabled_advisor_stays_silent_without_an_error(
        self, facade, world, llm, proposal_response
    ):
        """
    ...
    async def test_personality_reaches_the_prompt(
        ...
        assert "Шаман, говорит загадками." in llm.structured_calls[0]["system_prompt"]
...
class TestPendingProposals:
    ...
    async def test_offered_proposal_waits_for_the_player(
        ...
        proposal = await offer(facade, world, llm, proposal_response)
    ...
    @pytest.mark.asyncio
    async def test_silence_leaves_nothing_in_the_queue(
        self, facade, world, llm, silent_response
    ):
    ...
    async def test_leaving_the_party_forgets_the_advice(
        ...
        await offer(facade, world, llm, proposal_response)
    ...
    async def test_offered_event_carries_the_advisors_words(
        self, facade, world, llm, fake_bus, proposal_response
    ):
        proposal = await offer(facade, world, llm, proposal_response)
...
class TestAnswerProposal:
    ...
    async def test_refusal_closes_the_window_silently(
        ...
        proposal = await offer(facade, world, llm, proposal_response)
        decline = next(o for o in proposal.options if o.kind == AdvisorOptionKind.DECLINE)
    ...
    @pytest.mark.asyncio
    async def test_accepted_proposal_changes_nothing_until_the_skills_are_written(
        self, facade, world, llm, fake_bus, proposal_response
    ):
        """
        Навыков Function Calling еще нет: советник отвечает словами, но до
        мира его решение не доходит - и интерфейс это видит.
        """
        proposal = await offer(facade, world, llm, proposal_response)
...
# ==================================================================
# ИСПОЛНИТЕЛЬ С ПОДКЛЮЧЕННЫМ НАВЫКОМ
# ==================================================================


class TestExecutedActions:
    """
    Заглядываем в будущее: когда навыки появятся, фасад обязан провести их
    через исполнителя и рассказать об этом миру событием.
    """

    @pytest.mark.asyncio
    async def test_executed_action_is_published(
        self, llm, fake_bus, fake_prompt_builder, fake_context_builder,
        world, proposal_response, monkeypatch,
    ):
        executor = AdvisorActionExecutor()

        async def raise_taxes(world_state, faction_id, arguments) -> str:
            return "Налог поднят на 10%."

        executor.register("change_taxes", raise_taxes)

        facade = AdvisorFacade(
            llm_client=llm,
        ...
        # Подменяем заглушку выбора навыков: сегодня советник намерений не строит
        async def scripted_actions(*args, **kwargs):
            return "Будет исполнено.", [
                AdvisorAction(tool_name="change_taxes", arguments={"percent": 10})
            ]

        monkeypatch.setattr(facade._generator, "request_actions", scripted_actions)
        ...
        assert payload["tool_name"] == "change_taxes"
        assert payload["detail"] == "Налог поднят на 10%."
