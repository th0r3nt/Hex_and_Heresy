...
class TestAdvisorHandlers:
    ...
    async def test_player_always_gets_a_way_to_refuse(self, executor, advisor_facade, context):
...
class TestGameMasterHandlers:
    async def test_commander_joins_the_hiring_pool(self, executor, world, fake_bus, context):
        result = await executor.execute(
            tool_call(
                "create_commander",
                name="Ольгерд",
    ...
    async def test_invented_traits_do_not_reach_the_character(self, executor, world, context):
