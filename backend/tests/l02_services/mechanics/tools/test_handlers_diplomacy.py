...
class TestVerdicts:
    async def test_trade_action_creates_agreement(self, executor, world, fake_bus, audience):
    ...
    async def test_tribute_demand_is_recorded(self, executor, world, fake_bus, audience):
        await executor.execute(tool_call("demand_tribute", gold_amount=250.0), audience())
    ...
    async def test_pact_during_war_is_refused_by_the_domain(self, executor, world, audience):
        """
...
class TestForeignPolicy:
    ...
    async def test_tribute_is_paid_out_of_the_treasury(self, executor, world, audience):
        await executor.execute(tool_call("demand_tribute", gold_amount=200.0), audience())
...
class TestExecuteAmbassador:
    ...
    async def test_execution_without_an_ambassador_is_refused(self, executor, world, audience):
