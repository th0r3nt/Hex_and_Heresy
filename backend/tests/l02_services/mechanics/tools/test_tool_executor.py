...
from backend.l01_domain.llm.models.tools import ToolDefinition, ToolResult
...
from backend.tests.l02_services.fakes import tool_call
...
class TestExecute:
    ...
    async def test_unknown_tool_is_an_honest_refusal(self, register, bare_context):
        ...
        result = await register(handler).execute(tool_call("annex_everything"), bare_context)
    ...
    async def test_arguments_beyond_the_schema_are_rejected(self, register, bare_context):
        ...
        result = await register(handler).execute(tool_call("march", haste=99), bare_context)

        assert result.success is False
        assert "параметры" in result.error.lower()
        assert called is False, "Обработчик не должен видеть невалидные аргументы"

    async def test_domain_error_becomes_a_reason_for_the_model(self, register, bare_context):
        """
    ...
    async def test_unexpected_error_does_not_escape_the_executor(self, register, bare_context):
