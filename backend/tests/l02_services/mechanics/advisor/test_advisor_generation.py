...
from backend.l01_domain.exceptions.advisor import AdvisorGenerationFailedError
from backend.l01_domain.exceptions.llm import LLMRequestFailedError
from backend.l01_domain.factions.models.advisor import ADVISOR_MAX_OPTIONS
from backend.l01_domain.llm.tools.catalog import Toolset, get_toolset
from backend.l02_services.mechanics.advisor.generation import FREEFORM_OPTION_LABEL
from backend.tests.l02_services.fakes import reply, tool_call
...
class TestGenerateProposal:
    ...
    @pytest.mark.asyncio
    async def test_silent_advisor_does_not_open_a_window(self, generator, world, humans, llm):
    ...
    @pytest.mark.asyncio
    async def test_broken_arguments_do_not_open_a_window(self, generator, world, humans, llm):
        """
    ...
    @pytest.mark.asyncio
    async def test_broken_model_does_not_break_the_turn(self, generator, world, humans, llm):
        """
...
class TestOptions:
    ...
    async def test_extra_options_are_cut_to_fit_the_window(
        ...
        llm.script(reply("", proposal_call(options=[f"Вариант {i}" for i in range(10)])))
    ...
    async def test_freeform_option_of_the_model_is_not_duplicated(
        ...
        llm.script(reply("", proposal_call(options=["Принять", FREEFORM_OPTION_LABEL])))
...
class TestAnswerQuestion:
    ...
    @pytest.mark.asyncio
    async def test_broken_model_is_a_refusal_to_serve(self, generator, world, humans, llm):
        """
    ...
    @pytest.mark.asyncio
    async def test_empty_answer_is_a_refusal_to_serve(self, generator, world, humans, llm):
        llm.text_response = "   "
