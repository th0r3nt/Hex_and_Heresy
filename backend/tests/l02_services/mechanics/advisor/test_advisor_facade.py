...
from backend.l01_domain.exceptions.advisor import (
    AdvisorDisabledError,
    AdvisorOptionNotFoundError,
    AdvisorProposalNotFoundError,
)
from backend.l01_domain.exceptions.base import DomainError
from backend.l01_domain.exceptions.factions import FactionNotFoundError
from backend.l01_domain.factions.models.advisor import (
    AdvisorActionStatus,
    AdvisorOptionKind,
)
from backend.l01_domain.llm.tools.definitions.strategic import SET_TAX_RATE
...
class TestSettings:
    ...
    @pytest.mark.asyncio
    async def test_personality_reaches_the_prompt(self, facade, world, llm, proposal_call):
        llm.script(reply("", proposal_call()))
...
class TestPendingProposals:
    ...
    @pytest.mark.asyncio
    async def test_answered_proposal_leaves_the_queue(self, facade, world, llm, proposal_call):
        proposal = await offer(facade, world, llm, proposal_call)
...
class TestAnswerProposal:
    @pytest.mark.asyncio
    async def test_refusal_closes_the_window_silently(self, facade, world, llm, proposal_call):
