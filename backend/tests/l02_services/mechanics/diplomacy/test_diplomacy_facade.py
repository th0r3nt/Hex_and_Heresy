...
from backend.l01_domain.factions.models.diplomacy.pacts import TradeAgreement
...
class TestDiplomacyTick:
    ...
    @pytest.mark.asyncio
    async def test_pay_tribute_moves_gold_and_closes_demand(
        self, world, humans, elfs, fake_bus
    ):
        facade = DiplomacyFacade(event_bus=fake_bus)
        relation = world.get_or_create_relation("humans", "elfs")
...
class TestDiplomacyInsideStrategicTurn:
    ...
    async def test_dispatch_travels_across_global_ticks(self, world, fake_bus):
        facade = DiplomacyFacade(event_bus=fake_bus)
        orchestrator = StrategicTurnOrchestrator(diplomacy_facade=facade, event_bus=fake_bus)
