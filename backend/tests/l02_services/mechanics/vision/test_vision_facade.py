...
class TestWorldVisionRefresh:
    ...
    async def test_leaving_hex_turns_it_into_fog(
        ...
        assert (
            facade.get_hex_status(world, "humans", hex_at(6)) == HexVisibilityState.FOG_OF_WAR
        )
...
class TestVisionReading:
    def test_unknown_faction_sees_black_fog(self, facade: VisionFacade, world: WorldState):
        assert (
            facade.get_hex_status(world, "humans", hex_at(9)) == HexVisibilityState.UNEXPLORED
        )

    def test_mask_is_created_on_first_request(self, facade: VisionFacade, world: WorldState):
        """
