...
from backend.l01_domain.army.constants import EquipmentSlot, EquipmentTag
from backend.l01_domain.army.models.card.equipment import Equipment, EquipmentStats
from backend.l01_domain.exceptions.factions import InsufficientResourcesError
from backend.l01_domain.factions.constants import ResourceType
from backend.l02_services.mechanics.gunsmith.crafting import StatPriorities
from backend.l02_services.mechanics.gunsmith.facade import INCOMPLETE_ANSWER_REPLY
from backend.l02_services.mechanics.gunsmith.validation.balance import EquipmentBalancer
from backend.l02_services.mechanics.gunsmith.validation.economy import EquipmentEconomist
from backend.tests.l02_services.fakes import reply
...
class TestSystemPrompt:
    @pytest.mark.asyncio
    async def test_prompt_carries_role_economy_and_race(self, facade, world, llm, draft_call):
        """
    ...
    @pytest.mark.asyncio
    async def test_player_order_reaches_the_user_prompt(self, facade, world, llm, draft_call):
        llm.script(reply("", draft_call()))
...
class TestDraftApproved:
    @pytest.mark.asyncio
    async def test_draft_carries_the_masters_reply(self, facade, world, llm, draft_call):
        call = draft_call()
    ...
    async def test_stats_and_price_come_from_the_math_modules(
        ...
        expected_stats = EquipmentBalancer.normalize_stats(call.arguments["tier"], priorities)
