...
from backend.l01_domain.army.models.card.equipment import EquipmentStats
from backend.l01_domain.exceptions.army import InvalidEquipmentSlotError
from backend.l02_services.mechanics.gunsmith.blueprints import BlueprintRegistry
from backend.l02_services.mechanics.gunsmith.crafting import LLMGunsmithResponse
...
def construct(response: LLMGunsmithResponse, stats: EquipmentStats = STATS):
    return BlueprintRegistry.construct_draft(
        response, stats, cost_gold=31.2, cost_material=73.1
    )
...
class TestCategoryResolution:
    ...
    def test_category_from_a_foreign_slot_is_rejected_by_the_domain(self):
        ...
        """
        response = make_response(slot=EquipmentSlot.ARMOR, category_name="sword", tags=[])
