...
from backend.l01_domain.exceptions.world import RulerTemplateNotFoundError
...
from backend.l01_domain.maps.constants import STRATEGIC_MAP_TOTAL_HEXES
from backend.l01_domain.maps.models.strategic import (
    HexCoordinates,
...
from backend.l01_domain.world.models.setup import (
    FactionSetupConfig,
    NewGameConfig,
    RulerSetupConfig,
)
from backend.l01_domain.world.models.state import WorldState
from backend.l01_domain.world.models.victory import VictoryConditionConfig
...
def _fingerprint(world: WorldState) -> dict:
    ...
    return {
        ...
        "buildings": sorted(
            (f.race.value, tuple(sorted(_building_ids(f)))) for f in world.factions.values()
        ),
        "armies": sorted(
...
async def test_string_seed_is_accepted(generator: WorldGenerator, make_config: ConfigFactory):
...
async def test_bases_stand_on_standard_citadel_hexes(
    ...
    rival = next(f for f in world.factions.values() if f.race == FactionRace.GREENSKINS)

    assert player.capital_hex == north_base
    assert rival.capital_hex == south_base


async def test_no_two_sides_share_a_hex(generator: WorldGenerator, make_config: ConfigFactory):
...
async def test_barony_castle_stands_in_the_center_of_the_map(
    ...
    barony = next(f for f in world.factions.values() if f.race == FactionRace.BARONIAL_TROOPS)
...
async def test_all_lore_landmarks_stand_in_the_equatorial_belt(
    ...
    placed = [poi for poi in world.points_of_interest.values() if poi.blueprint.is_landmark]
...
async def test_procedural_points_are_scattered_over_neutral_hexes(
    ...
    procedural = [
        poi for poi in world.points_of_interest.values() if not poi.blueprint.is_landmark
    ]
...
async def test_every_side_sees_its_own_lands_and_nothing_of_the_enemy(
    ...
    assert all(world.is_hex_visible_to(player.id, coord) for coord in _occupied_hexes(player))
