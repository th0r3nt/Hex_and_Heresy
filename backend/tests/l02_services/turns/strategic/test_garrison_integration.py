...
from backend.l01_domain.combat.models.reports import (
...
class SpyTacticalOrchestrator:
    ...
    async def execute_turn(
        self, battle_state, squads, strategic_hex, **_
    ) -> TacticalTurnReport:
...
class TestGarrisonUpkeep:
    @pytest.mark.asyncio
    async def test_garrison_food_is_charged_with_the_discount(self, human_faction, fake_bus):
        """
...
class TestGarrisonDefendsItsLand:
    @pytest.mark.asyncio
    async def test_battle_on_the_base_hex_draws_in_the_garrison(self, human_faction, fake_bus):
        """
        ...
        await facade.execute_tactical_turn(world_state=world_state, battle_state=battle_state)
