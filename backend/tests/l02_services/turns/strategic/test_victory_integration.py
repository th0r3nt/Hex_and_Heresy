...
class TestVictoryInStrategicTurn:
    ...
    async def test_turn_report_carries_victory_progress(
        ...
        report = await StrategicTurnOrchestrator(event_bus=fake_bus).execute_turn(world_state)
...
class TestTurnsFacadeVictoryReading:
    def test_progress_is_readable_without_running_a_turn(self, human_faction, fake_bus):
        world_state = WorldState()
        world_state.add_faction(human_faction)
