...
from backend.l01_domain.exceptions.llm import (
    LLMAuthorizationError,
    LLMRequestFailedError,
)
from backend.l01_domain.llm.models.tools import ToolCall
from backend.l01_domain.llm.tools.catalog import Toolset, get_toolset
from backend.l01_domain.llm.tools.definitions.general import REPLY
from backend.l01_domain.llm.tools.definitions.strategic import ORDER_ARMY_MARCH, SET_TAX_RATE
from backend.l01_domain.llm.tools.schemas.strategic import SetTaxRateParams
from backend.l03_infrastructure.llm.executor import LLMExecutor
from backend.l03_infrastructure.llm.keys.rotator import APIKeyRotator
...
class TestRequest:
    async def test_tools_are_sent_as_function_schemas(self, build, answer):
        ...
        await executor.generate_with_tools(system_prompt="s", user_prompt="u", tools=TOOLS)
    ...
    async def test_whole_scene_toolset_is_accepted(self, build, answer):
        ...
        await executor.generate_with_tools(system_prompt="s", user_prompt="u", tools=toolset)
...
class TestBrokenArguments:
    ...
    async def test_arguments_that_are_not_an_object_are_dropped(self, build, answer, payload):
