...
from backend.l01_domain.army.models.card.unit import BaseUnitStats, UnitArchetype
from backend.l01_domain.army.models.characters.commanders import (
    Commander,
    CommanderGenerationType,
)
from backend.l01_domain.army.models.characters.heroes import Hero, HeroState
...
from backend.l01_domain.common import CharacterGenerationType, FactionRace
from backend.l01_domain.factions.constants import DiplomaticStance, ResourceType
from backend.l01_domain.factions.models.buildings import Headquarters
...
from backend.l01_domain.factions.models.lord import Lord
from backend.l01_domain.llm.models.context import ContextBlock
from backend.l01_domain.maps.models.strategic import HexCoordinates
from backend.l01_domain.protocols.llm import ContextBuilderProtocol
from backend.l01_domain.world.models.battle_log import BattleDossier
from backend.l01_domain.world.constants import GlobalEventCategory
...
class TestOtherRoles:
    ...
    def test_named_squad_tells_its_legend(
        ...
        assert "Маркус" in body(
            builder.build_veteran_context(world_state, squad), "Твое положение"
        )
...
class TestChronicler:
    def test_rumor_context_looks_at_the_whole_map(
        ...
        assert "Священная Империя против Орда Ржавых Клыков" in body(blocks, "Войны на карте")
...
class TestWarnings:
    def test_empty_treasury_is_flagged(
        self, builder: ContextBuilder, world_state: WorldState, faction: Faction
    ):
...
class TestRender:
    ...
    def test_role_context_renders_as_sections(
        ...
        rendered = builder.render(
            builder.build_lord_context(world_state, faction, counterpart)
        )
