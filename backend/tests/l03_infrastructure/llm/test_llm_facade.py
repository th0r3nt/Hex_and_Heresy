...
from backend.l03_infrastructure.llm import facade as facade_module
...
class ScriptedExecutor:
    ...
    async def generate_with_tools(self, **kwargs: Any) -> Any:
        ...
        return self._resolve((f"текст от {self.config.id}", [ToolCall(name="set_tax_rate")]))
...
class TestRouting:
    ...
    async def test_keyless_provider_gets_an_empty_rotator(self, facade: LLMFacade, local):
        ...
        assert (
            await facade.generate_text(system_prompt="s", user_prompt="u") == "текст от local"
        )
