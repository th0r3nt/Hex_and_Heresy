...
from backend.l01_domain.combat.visibility import (
...
class TestWeatherPenalty:
    ...
    def test_bad_weather_takes_its_cells(self, weather: WeatherCondition, expected: int):
...
class TestResolvedVisibility:
    def test_clear_grey_hours_give_full_range(self):
        resolved = resolve_visibility_range_cells(TimeOfDay.GREY_HOURS, WeatherCondition.CLEAR)

        assert resolved == DAY_VISION_RANGE_CELLS

    def test_neon_hours_cut_range_to_three_cells(self):
        resolved = resolve_visibility_range_cells(TimeOfDay.NEON_HOURS, WeatherCondition.CLEAR)
    ...
    def test_vision_is_never_zero(self, time_of_day: TimeOfDay, weather: WeatherCondition):
        assert resolve_visibility_range_cells(time_of_day, weather) >= MIN_VISION_RANGE_CELLS
