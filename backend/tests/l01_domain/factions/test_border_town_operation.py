...
@pytest.fixture
def town() -> BorderTown:
    settlement = BorderTown(faction_id="humans", name="Врата висельников", center_hex=CENTER)
...
def test_walking_past_the_town_brings_nothing(town: BorderTown):
    ...
    assert (
        border_town_resolution_loot(BorderTownResolutionType.IGNORE, town.invested_resources)
        == {}
    )
