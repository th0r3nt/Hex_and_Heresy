...
def _proposal() -> AdvisorProposal:
    return AdvisorProposal(
        ...
        options=[
            ...
            AdvisorOption(id="opt_soft", label="Поднять на 5%", kind=AdvisorOptionKind.ADJUST),
        ],
    )
...
def test_advice_without_a_party_is_a_conflict(client: TestClient, container: FakeContainer):
...
def test_choice_and_words_of_the_player_reach_the_facade(
    ...
    assert facade.answers == [("advp_taxes", "opt_soft", "Хватит и пяти процентов.")]
...
def test_empty_question_is_rejected_before_the_facade(
    ...
    response = client.post("/api/advisor/chat", json={"faction_id": "humans", "question": ""})
...
def test_advisor_can_be_switched_off(client: TestClient, container: FakeContainer):
