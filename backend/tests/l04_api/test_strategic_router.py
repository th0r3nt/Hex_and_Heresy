...
from backend.tests.l04_api.conftest import FakeContainer
...
def test_turn_requires_started_party(client: TestClient, container: FakeContainer):
    container.turns_facade = TurnsFacade()
...
def test_tax_rate_order_moves_the_slider_in_the_world(
    ...
    response = client.put(f"/api/strategic/factions/{faction.id}/tax-rate", json={"rate": 1.5})
...
def test_tax_rate_beyond_the_slider_is_rejected(
    ...
    response = client.put(f"/api/strategic/factions/{faction.id}/tax-rate", json={"rate": 3.0})
...
def _garrison(world_state: WorldState, faction: Faction, at: HexCoordinates) -> Garrison:
    ...
    garrison = Garrison(zone_id=hex_zone_id(at), faction_id=faction.id, hex_coordinates=at)
...
def test_victory_progress_needs_an_active_party(client: TestClient, container: FakeContainer):
    response = client.get("/api/strategic/victory-progress")
