...
from backend.l01_domain.world.constants import DifficultyLevel, GlobalEventCategory
...
def test_new_game_returns_a_playable_world(client: TestClient, container: FakeContainer):
    """
...
def test_new_game_respects_the_chosen_difficulty(
    ...
    response = client.post("/api/gameflow/new-game", json={"difficulty": difficulty.value})
...
def test_new_game_can_leave_baronies_out(client: TestClient, container: FakeContainer):
    response = client.post("/api/gameflow/new-game", json={"include_baronies": False})
...
def test_show_global_event_opens_screen(client: TestClient, active_party: WorldState):
    ...
    response = client.post("/api/gameflow/global-event/show", json={"event_id": event.id})
...
def test_show_unknown_global_event_answers_not_found(
    ...
    response = client.post("/api/gameflow/global-event/show", json={"event_id": "нет такого"})
...
def test_world_dependent_endpoint_requires_started_party(client: TestClient):
    ...
    """
    response = client.post("/api/gameflow/global-event/show", json={"event_id": "любой"})
