...
from backend.l01_domain.exceptions.saves import SaveNotFoundError
from backend.l01_domain.world.models.saves import SaveMetadata
from backend.l01_domain.world.models.state import WorldState
from backend.tests.l04_api.conftest import FakeContainer, FakeSession
...
def test_list_saves_returns_slots(client: TestClient, container: FakeContainer):
    ...
    assert response.json()["items"] == [{"id": "quicksave", "name": "Быстрое сохранение"}]
...
def test_load_missing_save_answers_not_found(client: TestClient, container: FakeContainer):
    facade = FakeSavesFacade()
    facade.missing_save_ids.add("нет-такого")
    container.saves_facade = facade
...
def test_delete_save_reports_missing_slot(client: TestClient, container: FakeContainer):
    facade = FakeSavesFacade()
    facade.missing_save_ids.add("нет-такого")
    container.saves_facade = facade
