...
def _start_battle(client: TestClient, world_state: WorldState) -> dict:
    ...
    for faction_id in ("humans", "elfs"):
        world_state.add_army(StrategicArmy(faction_id=faction_id, current_hex=BATTLE_HEX))
...
def test_current_battle_is_kept_between_requests(client: TestClient, active_party: WorldState):
    battle = _start_battle(client, active_party)
