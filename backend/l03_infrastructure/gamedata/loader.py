...
Считывает словари из пакета backend.gamedata, валидирует их через Pydantic
...
from backend import gamedata as gamedata_package
from backend.gamedata.common import RACES
from backend.l01_domain.army.models.card.equipment import Equipment
from backend.l01_domain.army.models.card.roster import RosterEntry
from backend.l01_domain.army.models.card.unit import UnitArchetype
from backend.l01_domain.common import FactionRace
from backend.l01_domain.factions.models.buildings import Building
from backend.l01_domain.factions.models.legendary import (
...
from backend.l01_domain.world.models.points_of_interest import PointOfInterestBlueprint
from backend.utils.logger import main_logger

# Корень пакета геймдаты берем у самого пакета, а не строкой: при переименовании
# или переносе каталога загрузчик поедет вместе с ним и не начнет молча
# пропускать все модули как "отсутствующие"
GAMEDATA_PACKAGE: Final[str] = gamedata_package.__name__

# Модуль с местами Ничьей земли: он один на весь мир, а не свой у каждой расы
POINTS_OF_INTEREST_MODULE: Final[str] = f"{GAMEDATA_PACKAGE}.world.points_of_interest"
...
def _load_faction_module(
    ...
    module_path = f"{GAMEDATA_PACKAGE}.{race.value}.{module_suffix}"
    try:
...
def _load_points_of_interest(registry: StaticGameDataRegistry) -> None:
    ...
    try:
        ...
        if e.name and POINTS_OF_INTEREST_MODULE.startswith(e.name):
            return
        main_logger.error(
            f"Ошибка импорта зависимостей внутри {POINTS_OF_INTEREST_MODULE}: {e}"
        )
        raise
    except Exception as e:
...
def _loader_commander(registry: StaticGameDataRegistry, race: FactionRace, raw: dict) -> None:
