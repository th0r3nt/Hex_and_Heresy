...
from backend.l03_infrastructure.databases.sql.management.chronicles import SQLChronicles
from backend.l03_infrastructure.databases.sql.management.saves import SQLSaves
            ...
            async with self._session_factory() as session:
                async with session.begin():
                    ...
                    if existing is not None:
                        main_logger.debug(f"Летопись боя '{battle_id}' уже записана, пропуск.")
                        return
