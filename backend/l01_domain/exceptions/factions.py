...
class GarrisonRotationForbiddenError(FactionError):
    ...
    def __init__(self, zone_id: str, reason: str) -> None:
        self.zone_id = zone_id
        self.reason = reason
        super().__init__(f"Ротация гарнизона земли '{zone_id}' невозможна: {reason}.")
...
class HexNotAdjacentToTownError(FactionError):
    ...
    def __init__(self, town_name: str, zone_id: str) -> None:
        self.town_name = town_name
        self.zone_id = zone_id
        super().__init__(f"Земля '{zone_id}' не граничит с пограничным городом '{town_name}'.")
...
class BorderTownResolutionInvalidError(FactionError):
    ...
    def __init__(self, town_id: str, reason: str) -> None:
        self.town_id = town_id
        self.reason = reason
        super().__init__(f"Судьбу пограничного города '{town_id}' решить нельзя: {reason}.")
