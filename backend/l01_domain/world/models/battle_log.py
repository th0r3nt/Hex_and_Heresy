...
from backend.l01_domain.combat.constants import TimeOfDay, WeatherCondition
from backend.l01_domain.common import FactionRace
from backend.l01_domain.world.constants import CHRONICLE_MASSACRE_LOSS_RATIO
...
class SquadBattleLog(BaseModel):
    ...
    is_named: bool = Field(
        default=False, description="Именной отряд (ветеран) или безымянное мясо"
    )
    commander_name: Optional[str] = Field(
        default=None, description="Имя командира именного отряда"
    )
...
class BattleTurningPoint(BaseModel):
    ...
    value: float = Field(
        default=0.0, description="Числовая мера события: погибшие, сила удара"
    )
...
class BattleDossier(BaseModel):
    ...
    finished_tick: Optional[int] = Field(
        default=None, ge=0, description="Раунд, оборвавший бой"
    )
