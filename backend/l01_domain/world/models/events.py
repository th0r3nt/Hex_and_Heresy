...
from backend.l01_domain.common import MechanicalModifier
...
class GlobalEvent(BaseModel):
    ...
    а также может порождать враждебные армии.
    """
    ...
    # Физические последствия на глобальной карте
    spawned_army_id: Optional[str] = Field(
        default=None,
        description="ID созданной событием армии на глобальной карте (например, бунтовщиков или монстров)",
    )
    spawn_hex: Optional[HexCoordinates] = Field(
        default=None,
        description="Координаты гекса появления сил или эпицентра события",
    )
