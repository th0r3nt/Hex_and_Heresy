...
from backend.l01_domain.exceptions.advisor import AdvisorOptionNotFoundError
...
class AdvisorProposal(BaseModel):
    ...
    tick: int = Field(default=0, ge=0, description="Глобальный такт, на котором подан совет")
...
class AdvisorDecision(BaseModel):
    ...
    def has_unsupported_actions(self) -> bool:
        ...
        """
        return any(
            outcome.status == AdvisorActionStatus.NOT_SUPPORTED for outcome in self.outcomes
        )
