...
class ClaimBorderLandRequest(BaseModel):
    ...
    faction_id: str = Field(..., min_length=1, description="Владелец города")
    target_hex: HexCoordinates = Field(..., description="Гекс, вплотную примыкающий к городу")
...
class BorderTownOperationResponse(BaseModel):
    ...
    @classmethod
    def from_operation(cls, operation: BorderTownOperation) -> "BorderTownOperationResponse":
...
class GarrisonResponse(BaseModel):
    ...
    def from_garrison(cls, garrison: Garrison) -> "GarrisonResponse":
        ...
        return cls(
            ...
            militia_squads=[GarrisonSquadView.from_squad(s) for s in garrison.militia_squads],
            stationed_squads=[
