...
from backend.l04_api.dependencies import Turns, World
from backend.l04_api.http.schemas.common import OperationResult
from backend.l04_api.http.schemas.strategic import (
...
async def get_hex_visibility(
    ...
    return HexVisibilityResponse(
        ...
        state=turns.get_hex_visibility(world_state=world, faction_id=faction_id, coord=coord),
    )
...
@router.get("/factions/{faction_id}/border-towns", response_model=list[BorderTownResponse])
...
@router.post("/border-towns/{town_id}/resolve", response_model=BorderTownOperationResponse)
...
@router.get("/border-towns/{town_id}/operation", response_model=BorderTownOperationResponse)
