...
from backend.utils.event.bus import resolve_event_name
...
class EventDispatcher:
    ...
    def _to_jsonable(self, value: Any) -> Any:
        ...
        if isinstance(value, dict):
            converted = {str(key): self._to_jsonable(item) for key, item in value.items()}
            return {
                key: item for key, item in converted.items() if item is not _UNSERIALIZABLE
            }
