...
from backend.l01_domain.combat.constants import FacingAngle, ReactionType
...
from backend.l01_domain.exceptions.chronicler import BattleDossierNotFoundError
from backend.l01_domain.maps.constants import ALLIED_LANDS_RING_RADIUS
from backend.l01_domain.maps.models.strategic import HexCoordinates, hex_distance
from backend.l01_domain.world.battle_summary import render_battle_summary
from backend.l01_domain.world.constants import CHRONICLE_CHAIN_PANIC_SQUADS
from backend.l01_domain.world.models.battle_log import (
    BattleDossier,
    BattleSide,
    BattleTurningPoint,
    SquadBattleLog,
...
class BattleLogCollector:
    ...
    def absorb_turn(self, report: TacticalTurnReport) -> BattleDossier:
        ...
        wiped_before = {squad_id for squad_id, log in dossier.squads.items() if log.wiped_out}
    ...
    def _absorb_charges(self, dossier: BattleDossier, report: TacticalTurnReport) -> None:
        for charge in report.charge_reports:
            ...
            if self._is_line_broken(
                charge.reaction, charge.attacker_deaths, charge.defender_deaths
            ):
    ...
    def _absorb_morale(self, dossier: BattleDossier, report: TacticalTurnReport) -> None:
        ...
        if morale.chain_panic_shocks or panicked_count >= CHRONICLE_CHAIN_PANIC_SQUADS:
            names = [
                name
                for name in (
                    self._name_of(dossier, squad_id) for squad_id in morale.panicking_squad_ids
                )
                if name
            ]
            dossier.add_turning_point(
                BattleTurningPoint(
                    ...
                    details=(
                        "бегство перекинулось на соседей: " + ", ".join(names)
                        if names
    ...
    def _dominant_faction(self, dossier: BattleDossier, side: BattleSide) -> Optional[str]:
        """
