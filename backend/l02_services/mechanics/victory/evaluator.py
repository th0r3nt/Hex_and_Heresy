...
from backend.l02_services.mechanics.victory.narrative import (
    describe_defeat,
...
class VictoryEvaluator:
    ...
    def calculate_progress(self, world_state: WorldState, faction_id: str) -> VictoryProgress:
        """
    ...
    def _is_wiped_out(world_state: WorldState, faction: Faction) -> bool:
        ...
        """
        has_armies = any(
            not army.is_wiped_out for army in world_state.get_faction_armies(faction.id)
        )
        has_lands = bool(
    ...
    def evaluate(self, world_state: WorldState) -> VictoryEvaluationResult:
        ...
        if player_victory is not None:
            return VictoryEvaluationResult(
                ...
                reason=describe_player_victory(player_victory, player, progress[player.id]),
                progress=progress,
            )
