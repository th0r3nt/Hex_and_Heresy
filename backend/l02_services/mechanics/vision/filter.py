...
from backend.l01_domain.world.constants import GlobalEventScope
...
class VisionFilter:
    ...
    def filter_world_for_faction(self, world_state: WorldState, faction_id: str) -> WorldState:
        """
    ...
    @staticmethod
    def _trim_faction_territory(faction: Faction, vision_map: FactionVisionMap) -> None:
        """
        Оставляет чужой фракции только ту застройку, что стоит на разведанных гексах.
        """
        faction.border_towns = [
            town for town in faction.border_towns if vision_map.is_explored(town.center_hex)
        ]
