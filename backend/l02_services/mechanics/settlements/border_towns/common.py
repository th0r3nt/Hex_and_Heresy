...
def require_town_on_map(world_state: WorldState, town_id: str) -> tuple[Faction, BorderTown]:
    """
...
def assert_hex_is_free(
    ...
    if world_state.get_point_of_interest_at(coord) is not None:
        raise InvalidSettlementPlacementError(zone_id, "гекс занят ориентиром Ничьей земли")
